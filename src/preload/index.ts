...
const api = {
  ...
  getGlobalTimeHeatmap: (): Promise<{ year: number; month: number; total: number; contacts: number }[]> => ipcRenderer.invoke('get-global-time-heatmap'),
  getTopContactsMonth: (year: number, month: number): Promise<{ chat_identifier: string; message_count: number; from_me: number; from_them: number; avg_heat: number }[]> => ipcRenderer.invoke('get-top-contacts-month', year, month),
  getPersonTimeline: (chatId: string): Promise<{ year: number; month: number; message_count: number; avg_heat: number }[]> => ipcRenderer.invoke('get-person-timeline', chatId),
